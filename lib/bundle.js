	...
	GameMap.prototype.renderBombs = function (ctx) {
	  this.bombs.forEach( (bomb, index) => {
	    if (bomb.time <= bomb.explodeTime) {
	      ...
	      ctx.drawImage(bombImage, sx, 0, 16, 16, bomb.mapPos[0] * this.tileSize, bomb.mapPos[1] * this.tileSize, 16, 16);
	    } else {
	...
	GameMap.prototype.renderCharacters = function (ctx) {
	  for (var i = 0; i < this.characters.length; i++) {
	    if (this.characters[i].playerNum === 1) {
	      ...
	      ctx.drawImage(bomber, sx, 0, 17, 23, character1.pos[0]-2 ,character1.pos[1]-12, 17, 23);
	      ...
	      ctx.drawImage(bomber2, sx, 0, 17, 23, character2.pos[0]-2 ,character2.pos[1]-12, 17, 23);
	    }
	  }
	};
